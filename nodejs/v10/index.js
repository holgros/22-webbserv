...
app.get("/users", function(req, res) {
    ...
    if (Object.keys(req.query).length == 0) {  // kolla om objektet är tomt
    ...
    let output = [];
    for (let key in req.query) {    // loopa genom attributen
        for (let i = 0; i < users.length; i++) {     // loopa genom fältet users
            if (users[i][key] == req.query[key]) {   // om vi har en matchning
                output.push(users[i]);
            }
        }
    }
